...
angular
    ...
    .controller('dashboard', ['$scope', '$http', '$rootScope', function ($scope, $http, $rootScope) {
        ...
        function drawBlocks() {
            ...
            var dataExchange = blockData.block.EXCHANGE;
            ...
            var noOfSerialComponents = 4;
            var noOfGaps = 2 * noOfSerialComponents;
            ...
            var shExchange = (height - dataExchange.length * rh) / (dataExchange.length + 1);

            var links = [];
            var linkStateToClasses = {"CONNECTED": "link-connected", "CLOSED": "link-closed", "CONNECTING": "link-connecting"};

            function calculateLinks(source, target, shSource, shTarget) {
                for (i = 0; i < source.length; i++) {
                    for (j = 0; j < target.length; j++) {
                        sourceX = (((width - noOfSerialComponents * rw) / noOfGaps) + (width / noOfGaps) * (source[i].x - 1)) + rw;
            ...
            var gExchange = svg.selectAll('.exchangeModule')
                .data(dataExchange)
                .enter()
                .append("g")
